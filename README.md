...
`PASS` означает успешную конкретную проверку, `FAIL` — ошибку, `SKIP` — цель не дала проверяемого результата. Пропущенная проверка не считается пройденной, но сама по себе не делает команду ошибочной; ненулевой код возвращается при наличии `FAIL`.
