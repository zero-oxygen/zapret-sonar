...
| Команда | Назначение |
|---|---|
| `sonar check [--json\|--json-v2]` | Проверить HTTP/CDN-цели; JSON v1 сохраняет агрегаты, v2 добавляет категории и отдельные результаты |
...
`PASS` означает успешную конкретную проверку, `FAIL` — ошибку, `NOT CHECKED` — проверка не дала осмысленного результата. Непроверенная цель не считается пройденной, но сама по себе не делает команду ошибочной; ненулевой код возвращается при наличии `FAIL`. `--json` сохраняет совместимую schema v1 с полем `skipped`, а `--json-v2` возвращает `not_checked` и результаты категорий `http`, `content`, `speed`. UDP и QUIC пока не входят в scope.
